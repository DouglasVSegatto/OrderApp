...
public class ProductController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<Product> updateProduct(@PathVariable Long id, @RequestBody UpdateProductDTO updateProductDTO) throws ProductNotFoundException {
        Product product = productService.updateProductById(id, updateProductDTO.getField(), updateProductDTO.getValue());
        return ResponseEntity.status(201).body(product);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Product> putProduct(@PathVariable Long id, @RequestBody ReplaceProductDTO replaceProductDTO) throws ProductNotFoundException {
